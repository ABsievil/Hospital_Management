...
async function fetchedAPI() {
    ...
    if (fetchedData1 && fetchedData1.data) {
        ...
        let patient = new Map();
        let totalRating = 0;
        fetchedData1.data.forEach((treatmentHistory) => {
    ...
    if (fetchedData2 && fetchedData2.data) {
        let totalCost = 0;
        let count = 0;
        let patientCount = 0;
        let patient = new Map();
        let totalRating = 0;
        fetchedData2.data.forEach((treatmentHistory) => {
            count++;
            patient.set(
                treatmentHistory.patientId,
                (patient.get(treatmentHistory.patientId) || 0) + 1
        ...
        if (patientCount <= 0) {
            ...
            const icon = document.createElement('i');
            numPatientStat.textContent =
                (((newPatient - patientCount) / patientCount) * 100).toFixed(
                    2
                ) + '%';
            icon.className = 'bx bx-up-arrow-alt';
            numPatientStat.prepend(icon);
            ...
            const icon = document.createElement('i');
            numPatientStat.textContent =
                (((patientCount - newPatient) / patientCount) * 100).toFixed(
                    2
                ) + '%';
        ...
        if (totalCost == 0) {
            ...
            revenueStat.textContent =
                (((newRevenue - totalCost) / totalCost) * 100).toFixed(2) + '%';
            const icon = document.createElement('i');
            icon.className = 'bx bx-up-arrow-alt';
            revenueStat.prepend(icon);
        } else {
            revenueStat.textContent =
                (((totalCost - newRevenue) / totalCost) * 100).toFixed(2) + '%';
            const icon = document.createElement('i');
            icon.className = 'bx bx-down-arrow-alt';
            revenueStat.prepend(icon);
        }
        ...
        const oldReExam =
            patientCount == 0
                ? 0
                : ((reExamCount / patientCount) * 100).toFixed(2);
        if (oldReExam == 0) {
            ...
            reExamStat.textContent =
                (((newReExam - oldReExam) / oldReExam) * 100).toFixed(2) + '%';
            const icon = document.createElement('i');
            icon.className = 'bx bx-up-arrow-alt';
            reExamStat.prepend(icon);
        } else {
            reExamStat.textContent =
                (((oldReExam - newReExam) / oldReExam) * 100).toFixed(2) + '%';
            const icon = document.createElement('i');
            icon.className = 'bx bx-down-arrow-alt';
            reExamStat.prepend(icon);
        }
        ...
        if (oldRating == 0) {
            ...
            ratingStat.textContent =
                (((newRating - oldRating) / oldRating) * 100).toFixed(2) + '%';
            const icon = document.createElement('i');
            icon.className = 'bx bx-up-arrow-alt';
            ratingStat.prepend(icon);
        } else {
            ratingStat.textContent =
                (((oldRating - newRating) / oldRating) * 100).toFixed(2) + '%';
            const icon = document.createElement('i');
            icon.className = 'bx bx-down-arrow-alt';
            ratingStat.prepend(icon);
        }
